...
static NSString *const kDefaultServiceURLForFreeGeoIP = @"https://freegeoip.net/json/";
static NSString *const kDefaultServiceURLForTelize = @"https://www.telize.com/geoip/";
...
-(void)geocode:(void(^)(BOOL success, FCIPAddressGeocoder* coder))completionHandler
{
    if( _location != nil && [[_location.timestamp dateByAddingTimeInterval:60] timeIntervalSinceNow] > 0 )
    {
        if( completionHandler ){
            completionHandler( YES, self );
    ...
    [NSURLConnection sendAsynchronousRequest:_serviceRequest queue:_operationQueue completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if( connectionError == nil )
        {
            ...
            if( httpResponse.statusCode == 200 )
            {
                ...
                if( JSONError == nil )
                {
                    ...
                    {
                        case FCIPAddressGeocoderServiceFreeGeoIP:
                            _locationZipCode = [_locationInfo objectForKey:@"zip_code"];
                            _locationCity = [_locationInfo objectForKey:@"city"];
                            _locationRegion = [_locationInfo objectForKey:@"region_name"];
                        ...
                        case FCIPAddressGeocoderServiceTelize:
                            _locationZipCode = [_locationInfo objectForKey:@"postal_code"];
                            _locationCity = [_locationInfo objectForKey:@"city"];
                            _locationRegion = [_locationInfo objectForKey:@"region"];
        ...
        if( _error != nil && _canUseOtherServicesAsFallback && [_servicesQueue count] > 0 )
        {
            ...
            {
                _completionHandler( _error == nil, self );
            }
        }
    }];
}
